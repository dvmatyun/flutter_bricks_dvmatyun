...
class SocketTopicImpl implements ISocketTopic {
  ...
  SocketTopicImpl.path({required String fullPath}) {
    var posLeft = 0;
    while (fullPath.length > (posLeft + 1) && fullPath.substring(posLeft, posLeft + 1) == pathDelimiter) {
      posLeft++;
    }
    var posRight = fullPath.length;
    while (posRight > 1 && fullPath.substring(posRight - 1, posRight) == pathDelimiter) {
      posRight--;
    }
  ...
  SocketTopicImpl.duo({required String host, required String topic1}) : this(orderedPathSegments: [host, topic1]);
